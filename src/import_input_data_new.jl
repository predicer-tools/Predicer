...
function main()
    ...
    for i in 1:nrow(system_data["processes"])
        ...
        processes[p.process] = Process(p.process, Bool(p.is_cf), Bool(p.is_online), Bool(p.is_res), p.eff, p.conversion, p.load_min, p.load_max, p.ramp_down, p.ramp_up, p.start_cost, p.min_online, p.min_offline)
        if Bool(p.is_cf)
            for s in keys(scenarios)
        ...
        if p.conversion == 1
            for so in sources
                push!(processes[p.process].topos, Topology(so[1], p.process, so[2], so[3], so[4], so[5]))
            end
            for si in sinks
                push!(processes[p.process].topos, Topology(p.process, si[1], si[2], si[3], si[4], si[5]))
            end
        elseif p.conversion == 2
            for so in sources, si in sinks
                push!(processes[p.process].topos, Topology(so[1], si[1], min(so[2], si[2]), si[3], si[4], si[5]))
            end
        elseif p.conversion == 3
            for so in sources, si in sinks
                push!(processes[p.process].topos, Topology(so[1], si[1], min(so[2], si[2]), si[3], si[4], si[5]))
                push!(processes[p.process].topos, Topology(si[1], so[1], min(so[2], si[2]), si[3], si[4], si[5]))
            end
        end
    end
    ...
    for i in 1:nrow(system_data["markets"])
        ...
        markets[mm.market] = Market(mm.market, mm.type, mm.node, mm.direction, mm.realisation, mm.reserve_type)
        #
        for s in keys(scenarios)
